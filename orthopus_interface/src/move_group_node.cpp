...
void MoveGroupNode::initializeServices()
{
  ...
  move_service_ = n_.advertiseService("/orthopus_interface/move_groupe_node/move", &MoveGroupNode::callbackMove, this);
  get_state_service_ =
      n_.advertiseService("/orthopus_interface/move_groupe_node/get_state", &MoveGroupNode::callbackGetState, this);
}
...
bool MoveGroupNode::callbackMove(niryo_one_msgs::RobotMove::Request& req, niryo_one_msgs::RobotMove::Response& res)
{
  ...
  {
    /* stop the move group pending motion */
    state_mutex_.lock();
    state_ = 2;
    state_mutex_.unlock();
    res.status = 8000;
  }
  else if (req.cmd.cmd_type == 123)
  {
    /* flip the pinch */
    ...
    primitive.dimensions[0] = constrain_box_scale * 0.05;
    primitive.dimensions[1] = constrain_box_scale * 0.1;
    primitive.dimensions[2] = constrain_box_scale * 0.1;
    ...
    ocm.absolute_x_axis_tolerance = 0.17;
    ocm.absolute_y_axis_tolerance = 0.17;
